		...
		bool has_nonfinite() const
{
    for (const auto &layer : nodes)
    {
        ...
        {
            if (!std::isfinite(n.out)) return true;
            if (!std::isfinite(n.b)) return true;
            if (!std::isfinite(n.delta)) return true;
            for (auto wptr : n.w_in)
            {
                if (wptr == nullptr || !std::isfinite(*wptr)) return true;
            }
            for (const auto &wp : n.w_out)
            {
                if (!std::isfinite(wp.second)) return true;
            }
        }
    }
    for (const auto &wp : w_out)
    {
        if (!std::isfinite(wp.second)) return true;
    }
    // in/out vectors of doubles are usually safe; adjust checks if needed
    return false;
}
};
