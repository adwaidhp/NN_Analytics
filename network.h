...
public:
    ...
    // Helper for running network
    void set_input(const std::list<double>& inputs) {
        auto it = in.begin();
        auto val_it = inputs.begin();
        for (; it != in.end() && val_it != inputs.end(); ++it, ++val_it) {
            *it = *val_it;
        }
    }
    ...
    std::list<double> run(const std::list<double>& inputs) {
        set_input(inputs);
        forward_pass();
        return get_out_put();
    }
    ...
    void print_weight () const
    {
        for (auto l_id = nodes.begin (); l_id != nodes.end (); ++l_id)
        {
            ...
            {				
                ...
                printf ("]\nIn weights: [");
                for (auto index : n_id->w_in) printf ("%f ", *index);
                printf ("]\nOut weights: [");
                for (auto index : n_id->w_out) printf ("%f ", index.second);
                std::cout << "]" <<std::endl;
            }	
        }
    }		
